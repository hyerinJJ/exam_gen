...
출제 원칙:
- 구조: 시나리오 3~4줄 + 소문항 1~3개. 소문항이 4개 이상이면 안 됨.
- 각 소문항은 하나의 적용 또는 분석 작업만 요구할 것. "분석하고 평가하고 제시하시오"처럼 한 문장에 여러 작업을 이어 묻지 말 것.
- 채점 포인트가 여럿이면 반드시 (1) (2) (3) 소문항으로 분리할 것.
...
- 반드시 단일 사실 하나만 담는다. "A이고 B이다", "A지만 B이다", "A와 B는 모두~"처럼 두 사실을 동시에 판단해야 하는 복합 명제는 절대 금지. 황당한 F와 동일 개념 반복도 금지.
