...
출제 원칙:
- 구조는 "시나리오 2~3문장 + 소문항 2개"를 기본으로 한다. 꼭 필요한 경우에만 소문항 3개까지 허용한다.
- 전체 문제는 시나리오와 소문항을 포함해 6줄 이내로 작성한다.
- 시나리오는 배경, 갈등, 판단 상황만 담고 세부 조건을 길게 나열하지 않는다.
- 강의 자료와 다른 새로운 맥락을 사용하되, 시나리오는 간결하게 유지한다.
...
메타데이터 반영 지침:
- knowledge_type=number: 수치·공식·정확한 값을 직접 묻는 문제
...
- knowledge_type=causal: 원인·결과 구조가 있는 시나리오 문제
