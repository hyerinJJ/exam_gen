...
수치값 암기 문제는 단답형으로 출제하지 마세요. 수식·계산 구조가 필요한 내용은 서술형에서 다룹니다.
각 문제를 만들 때 정답도 함께 작성하세요. answer는 그 문제의 실제 정답이어야 하며, 출제 계획의 핵심 개념을 기계적으로 복사하지 말고 문제 문장에 정확히 대응해야 합니다.

출제 원칙:
- 핵심 개념을 응축한 약어·용어·정확한 명칭을 묻는 문제를 우선할 것
- {count}개 중 1~2개는 유사 용어 구별, 약어 세부 등 변별 문제로 구성할 것
- 절차·순서가 있는 개념은 "몇 번째 단계" 또는 "직전/직후 단계"를 직접 묻는 문제를 출제할 것
- 같은 개념이나 같은 개념군·출제 범주를 두 문제에서 반복 출제하지 말 것
- 문제 지시문은 한국어로 작성할 것. 영어 전문용어·약어·프레임워크명 원어 병기는 허용한다.
- 마크다운 기호(**,##,*,#,__)를 절대 사용하지 마시오.

나쁜 예시 (절대 금지): 나열 요구("~을 3가지 쓰시오"), 서술 요구("정의를 설명하시오"), 같은 개념군 반복
정답 노출 금지: 문제 본문이나 빈칸 주변 문맥에서 정답이 직접 드러나거나 유추 가능한 단서를 절대 포함하지 마시오.
  예시 금지 패턴: "___는 X의 특성이다 (X를 구성하는 Y, Z, [빈칸] 중 하나)" — 선지가 문제 안에 나열됨
  예시 금지 패턴: "ABC(Abundant Bright Computing)의 약자는?" — 괄호 안에 이미 전체 명칭 노출

반드시 아래 JSON 배열 형식으로만 응답하세요:
[{{"id": "Q1", "type": "short", "question": "...", "answer": "..."}}]"""
...
- 채점 포인트가 여럿이면 반드시 (1) (2) (3) 소문항으로 나눌 것. 각 소문항은 하나의 사고 작업만 요구하고 독립적으로 부분점수를 줄 수 있어야 한다.
- 한 문장 안에 관계·차이·의의·한계를 줄줄이 이어 묻지 말 것. "설명하고 비교하고 제시하시오" 형태는 금지.
- 같은 개념이나 같은 개념군·출제 범주를 두 문제에서 반복 출제하지 말 것.
- 문제 지시문은 한국어로 작성할 것. 영어 전문용어·약어·프레임워크명 원어 병기는 허용한다.
- 마크다운 기호(**,##,*,#,__)를 절대 사용하지 마시오.
- 정답 노출 금지: 문제 본문이나 소문항에서 정답이 직접 드러나거나 정답을 유추할 수 있는 단서를 포함하지 마시오.
  예시 금지 패턴: "설계의 첫 번째 단계인 개념 설계란 무엇인가?" — '개념 설계'가 이미 질문에 노출됨
  예시 금지 패턴: 소문항 (2)가 소문항 (1)의 정답을 전제로 사용하는 경우
...
출제 원칙:
- 구조는 "상황 묘사 2~3문장 + 소문항 2개"를 기본으로 한다. 소문항 1~3개 범위에서만 허용하며, 꼭 필요한 경우에만 소문항 3개까지 쓴다.
- 전체 문제는 상황 묘사와 소문항을 포함해 6줄 이내로 작성한다.
- 상황 묘사는 배경, 갈등, 판단 상황만 담고 세부 조건을 길게 나열하지 않는다.
- 강의 자료와 다른 새로운 맥락을 사용하되, 상황 묘사는 간결하게 유지한다.
- 문제 본문에 "시나리오"라는 단어를 절대 사용하지 말 것. "다음 상황을 읽고", "다음 사례를 보고" 등 자연스러운 표현을 사용할 것.
- 각 소문항은 반드시 한 줄로 작성하고, 하나의 적용 또는 분석 작업만 요구한다.
- 소문항이 여러 채점 포인트를 요구해야 한다면 (1), (2), (3)으로 분리한다.
- "분석하고 평가하고 제시하시오"처럼 한 소문항 안에 여러 작업을 이어 묻지 않는다.
- 기본 소문항 구성은 다음을 따른다.
  (1) 수업 개념·프레임워크를 적용하여 상황을 분석하는 문항
  (2) 분석 결과를 바탕으로 개선 방향 또는 판단 근거를 제시하는 문항
- 같은 개념이나 같은 개념군·출제 범주를 두 문제에서 반복 출제하지 않는다.
- 문제 지시문은 한국어로 작성한다. 영어 전문용어·약어·프레임워크명 원어 병기는 허용한다.
- 마크다운 기호(**,##,*,#,__)를 절대 사용하지 않는다.
...
메타데이터 반영 지침:
- knowledge_type=number: 수식·계산 구조가 필요한 내용이므로 단답형에서 직접 값을 묻지 말 것
- knowledge_type=term: 정확한 명칭·약어·정의를 확인하는 문제
- knowledge_type=procedure: 단계·순서를 묻는 문제
- difficulty=hard: 유사 용어 구별, 약어 세부 등 변별 포인트
- importance=core: 강의 핵심 개념, 반드시 제대로 된 문제를 출제할 것
...
메타데이터 반영 지침:
- knowledge_type=causal: 원인·결과 관계 서술을 요구하는 문제
- knowledge_type=comparison: 두 개념 비교·대비를 요구하는 문제
- knowledge_type=framework: 의미·의의·한계·체계 서술을 요구하는 문제
- difficulty=hard: 비판적 분석이나 종합적 이해가 필요한 심층 질문
- importance=core: 반드시 포함해야 할 강의 핵심 개념
...
메타데이터 반영 지침:
- knowledge_type=procedure: 프로세스를 새로운 상황에 적용하는 문제
- knowledge_type=causal: 원인·결과 구조가 있는 상황 묘사 문제
- knowledge_type=framework: 프레임워크를 새로운 맥락에 적용하는 문제
- knowledge_type=case: 실제 사례를 분석하는 문제
- difficulty=hard: 기술·인간·조직 간 갈등이 있는 복잡한 상황
- importance=core: 강의 핵심 개념이 반드시 답안의 근거가 되어야 함
...
- 문제 지시문은 한국어로 작성한다. 영어 전문용어·약어·프레임워크명 원어 병기는 허용한다.
- 마크다운 기호(**,##,*,#,__)를 절대 사용하지 않는다.
- 정답 노출 금지: 상황 묘사나 소문항 안에 정답이 직접 드러나거나 유추 가능한 단서를 포함하지 않는다.
  예시 금지 패턴: "A사는 X 방식이 비효율적이라고 판단했다. (1) X의 문제점을 분석하시오." — 문제 자체가 정답 방향을 노출함
...
각 문제를 만들 때 answer도 함께 작성하세요. answer는 반드시 T 또는 F여야 합니다.

출제 원칙:
- 같은 개념이나 같은 개념군·출제 범주를 반복하지 않는다.
- 모든 문항은 "그럴듯한 오답(F)" 또는 "의심스럽지만 맞는 정답(T)"으로 구성한다.
- 황당하거나 명백히 틀린 F 문제는 만들지 않는다. 강의 맥락에서 실제로 헷갈릴 만한 명제여야 한다.
- "~할 수 있다", "~경우도 있다"처럼 지나치게 약해서 내용과 무관하게 참이 되기 쉬운 표현은 피한다.
- 강의에서 실제 사용된 용어를 유지하고, 문제 지시문은 한국어로 작성한다. 영어 전문용어·약어·프레임워크명 원어 병기는 허용한다.
- 문항 끝은 반드시 "(T/F)"로 통일한다.
- 마크다운 기호(**,##,*,#,__)를 절대 사용하지 않는다.
...
- 각 문제를 만들 때 answer도 함께 작성하세요. answer는 반드시 intended_answer와 같은 T 또는 F여야 합니다.
...
- 문항 끝은 반드시 "(T/F)"로 통일한다.
- 마크다운 기호(**,##,*,#,__)를 절대 사용하지 않는다.
- 정답 노출 금지: 명제 문장 안에 "이는 틀렸다", "이는 올바르다"처럼 정답을 직접 시사하는 표현을 포함하지 않는다.
...
def _format_plan_items_text(plan_items: list) -> str:
    ...
    for i, item in enumerate(plan_items, 1):
        ...
        # 새 필드 우선, 없으면 구형 필드로 fallback
        kt = meta.get("knowledge_type") or f"{meta.get('specificity', '-')}/{meta.get('cognitive_type', '-')}"
...
def _inject_grading_seed(results: list, plan_items: list) -> None:
    ...
    for q, item in zip(results, plan_items):
        ...
        if q_type == "short_answer":
            ...
            seed: dict = {
                "expected_answer": q.get("answer") or item.get("intended_answer", ""),
        ...
        q["topic_meta"] = dict(item.get("topic_meta", {}))
