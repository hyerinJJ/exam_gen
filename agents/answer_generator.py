...
포인트 3~4개, 합계 {{total_pts}}점. 핵심 포인트에 높은 점수. 점수는 정수만.
...
6) 서술형:
포인트별 중요도에 따라 점수를 다르게 배분하세요. 핵심 포인트 > 부수적 포인트. 합계 반드시 {{total_pts}}점. 점수는 정수만. 나누어 떨어지지 않으면 중요도 높은 포인트에 나머지 점수 추가.
[포인트 1]: 내용 (N점)
[포인트 2]: 내용 (N점)
[포인트 3]: 내용 (N점)
...
class AnswerGeneratorAgent(BaseAgentWorker):
    ...
    def _generate_rubric(self, q: dict, answer: str) -> str:
        pts = q.get("points", 10)
        prompt = RUBRIC_PROMPT.format(question=q["question"], answer=answer, total_pts=pts)
        response = retry_call(lambda: self._client.models.generate_content(model=FLASH_LITE, contents=prompt))
        return response.text.strip()
